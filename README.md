...
Anonymous activity measurement is optional and begins only after a visitor explicitly allows it. The browser sends only an allowlisted event name, a temporary random session ID, an optional opportunity ID, and a coarse page category—never an email address, search text, or selected filters. Events are deduplicated by day, raw events are blocked from browser reads, retained for 12 months, and summarized only for an explicitly designated impact administrator.
