...
const CONSENT_KEY='poh:impact-consent';
const SESSION_KEY='poh:impact-session';
const AnalyticsContext=createContext<AnalyticsValue|null>(null);
const analyticsConfigured=Boolean(supabase)&&import.meta.env.VITE_ANALYTICS_ENABLED==='true';
...
export function AnalyticsProvider({children}:{children:ReactNode}){
  ...
  const choose=useCallback((value:Exclude<Consent,'unknown'>)=>{try{localStorage.setItem(CONSENT_KEY,value)}catch{/* Browsing still works without storage. */}setConsent(value)},[]);
  ...
  const value=useMemo<AnalyticsValue>(()=>({enabled:analyticsConfigured,consent,allow:()=>choose('allowed'),decline:()=>choose('declined'),track}),[choose,consent,track]);
