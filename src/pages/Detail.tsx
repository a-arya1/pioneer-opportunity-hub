...
export default function Detail(){
  ...
  return <article className="detail"><div className="breadcrumb"><Link to="/opportunities">Opportunities</Link> / {o.category}</div><header className="detail-head"><div><p className="kicker">{o.category}{o.subcategory?` · ${o.subcategory}`:''}</p><h1>{o.title}</h1><p className="lede">{o.organization}</p><p>{o.summary}</p></div><aside><span className={`status ${o.status}`}>{o.status}</span><p><strong>{o.confidence} confidence</strong><br/>Last verified {o.lastVerified}</p><a className="button" href={o.sourceUrl} target="_blank" rel="noreferrer" onClick={sourceClick}>Go to official source</a><button className="button secondary" onClick={()=>void saves.toggle(o.id)}>{saves.saved.includes(o.id)?'Saved':'Save opportunity'}</button></aside></header>
    ...
    </div><aside className="source-box"><h2>Source record</h2><dl><dt>Publisher</dt><dd>{o.organization}</dd><dt>Source type</dt><dd>{o.sourceType}</dd><dt>Imported row</dt><dd>{o.sourceRow}</dd><dt>Confidence</dt><dd>{o.confidence}</dd></dl><a href={o.sourceUrl} target="_blank" rel="noreferrer" onClick={sourceClick}>Open official source</a><Link to="/submit?type=correction">Report outdated information</Link></aside></div><p className="detail-disclaimer">Details can change. Confirm eligibility, dates, cost, travel, and safety requirements on the official source before applying or attending.</p>
  </article>;
}
